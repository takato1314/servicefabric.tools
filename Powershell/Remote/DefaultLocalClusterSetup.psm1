...
function Set-LocalClusterReady([bool]$createOneNodeCluster = $False, [bool]$createMeshCluster = $False, [bool]$isSecure = $False)
{
    <#
    .SYNOPSIS
    Sets up a default non-secure local dev cluster if there is no existing local cluster on the machine. If there exists a local cluster on the machine
    but it not running, it will start the local cluster. Returns boolean value indicating if explicit args need to be provided to connect to local cluster
    after the function successfully returns.
    #>
    Get-Module -ListAvailable -Refresh --quiet *>$null

    if($createMeshCluster -eq $True -and !(IsMeshSDKInstalled)) 
    {
    ...
    {
        Write-Host "--------------------------------------------"
        Write-Host "Local Service Fabric Cluster is not setup..."
        Write-Host "Please wait while we setup the Local Service Fabric Cluster. This may take few minutes..."
            
        EnsureAdminPrivileges "Not running as administrator. You need to run Visual Studio with administrator privileges to setup the local cluster."
        
        ## Clean existing cluster just in case to ensure we have
        ## a clean state on machine before setting up new cluster
        try 
        {
            CleanExistingClusterShallow
        }
        catch [System.Exception]
        {
            Write-Warning "CleanExistingClusterShallow throws exceptions"
        ...
        $clusterRoots = SetupDataAndLogRoot -jsonFileTemplate $jsonFileTemplate -isAuto $True

        if ($isSecure -eq $true -or $createMeshCluster -eq $true) { InstallCertificates }
        
        DeployNodeConfiguration -clusterDataRoot $clusterRoots[0] -clusterLogRoot $clusterRoots[1] -isSecure $isSecure -createOneNodeCluster $createOneNodeCluster -createMeshCluster $createMeshCluster
        StartLocalCluster
        
        $connParams = GetConnectionParameters -isSecure $isSecure
        ...
        {
            DeployAddonServices -connParams $connParams -waitTime 120 -oneNodeCluster $createOneNodeCluster
        }

        Write-Host "Local Service Fabric Cluster created successfully."
        Write-Host "--------------------------------------------------"
    }
    elseif(!(IsLocalClusterRunning))
    {
        Write-Host "--------------------------------------------"
        Write-Host "Local Service Fabric Cluster is not running..."
        Write-Host "Please wait while we start the Local Service Fabric Cluster. This may take few minutes..."

        EnsureAdminPrivileges "Not running as administrator. You need to run Visual Studio with administrator privileges to start the local cluster."
        ...
        Write-Host "Local Service Fabric Cluster is running now."
        Write-Host "----------------------------------------"
    }
...
Export-ModuleMember -Function Set-LocalClusterReady
# SIG # Begin signature block
# MIIkWwYJKoZIhvcNAQcCoIIkTDCCJEgCAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAJeU19rHMbGa00
# vp6j3GJ6CD5TJaMw/etyhcPwfWLK66CCDYEwggX/MIID56ADAgECAhMzAAABUZ6N
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQ